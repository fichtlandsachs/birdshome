...
class BirdVideoAnalyzer:
    ...
    def __init__(self, session: Session, log_location: str, log_format: str, log_level: int, model_path="yolov8n.pt", frame_interval=30, resize_factor=0.5):
        """
        ...
        """
        self._logger = BirdshomeLogger('BirdVideoAnalyzer', location=log_location, logformat=log_format, level=log_level)
    ...
    def detect_bird_in_video(self, video_path):
        ...
        for frame_num in range(0, total_frames, int(self._frame_interval)):
            ...
            results = self._model.predict(source=frame, stream=False)
            for r in results:
