...
class ScreenShotHandler:
    ...
    def _update_config(self):
        ...
        self._last_run_screen_shot = self._dbHandler.get_config_entry(constants.REPLAY, constants.REPLAY_LAST_RUN_SCREEN)
        self._daysReplay = self._dbHandler.get_config_entry(constants.REPLAY, constants.REPLAY_DAYS)
        self._last_run_replay = self._dbHandler.get_config_entry(constants.REPLAY, constants.REPLAY_LAST_STARTTIME)
    ...
    def _create_screen_shot(self):
        ...
        if len(image.shape) > 2:
            ...
            image_gray = image
        cv2.imwrite(str(full_filename), image_gray)
        client_socket.close()
    ...
    def start_replay(self):
        ...
        while True:
            if self._active == 1:
                if self._last_run_replay is None:
                ...
                next_replay_date = datetime.datetime.strptime(self._last_run_replay,
